...
#'
#' @description  This package predicts whether the stock price at tommorow's market close to predicting results
#'
#' @param symbol
#'
#' @return NULL
#'
#' @example stock_predict('AAPL')
#'
#' @export
...
#To ignore the warning during usage
  options(warn = -1)
...
#Training model
  model<-stats::glm(up_down~data.Open+data.High+data.Low+data.Close+data.Volume+data.Adjusted+data.Adjusted.1+data.Adjusted.2+data.Adjusted.3+data.Adjusted.4+Day_of_month+Month_of_year+year+Day_of_week,family=binomial(link='logit'),data=train)
